  ...
  <div class="option-change-container">
    <a-row v-if="type === 'option' || type === 'tab'" :gutter="8">
      <div class="option-change-box" v-for="(val, index) in value" :key="index">
        <a-col :span="9"><Input v-model="val.label" @change="e => val.text = e.target.value" placeholder="名称"/></a-col>
        <a-col :span="9"><Input v-model="val.text" placeholder="名称" type="hidden"/></a-col>
