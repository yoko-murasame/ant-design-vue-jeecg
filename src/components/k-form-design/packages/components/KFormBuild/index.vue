  ...
  <a-config-provider :locale="locale">
      <a-form v-bind="$attrs" selfUpdate v-if="typeof value.list !== 'undefined' && typeof value.config !== 'undefined'
        ...
        <buildBlocks ref="buildBlocks" @handleReset="reset" v-for="(record, index) in value.list" :record="record"
          :dynamicData="getDynamicData" :config="config" :formConfig="value.config" :validatorError="validatorError"
          :key="index" @change="handleChange"  :disabled="disabled"/>
...
export default {
  ...
  methods: {
    ...
    async handleSubmit(e) {
      ...
      e && e.preventDefault();
      try { await this.beforeSubmit()} catch (ex) {
        console.error('KFormBuild::beforeSubmit', ex)
        this.$message.error('JS增强::beforeSubmit执行失败 ' + ex)
        throw ex
      }
      ...
      const callback = async (id) => {
        try { await that.afterSubmit({ ...formData, id  })} catch (ex) {
          ...
          that.$message.error('JS增强::afterSubmit执行失败 ' + ex)
          throw ex
        }
      };
