 ...
 * 1.自动根据接入List组件生成：formUrl、flowCode，特殊情况可自己覆盖
...
export default {
  data() {
    return {
       ...
       */
      formUrl: '', // (this.$options.__file || '').replace(/src\/views\/(.*)\/(.*?)List\.vue/, '$1/modules/$2Form'),
      // 流程定义id，默认与表名相同
      processDefinitionId: this.$options.name.replace(/\B([A-Z])/g, '_$1').toLowerCase().replace(/(.*?)_list/, '$1'),
      // 自定义流程处理接入
  ...
  created() {
    // 装载formUrl
    if (!this.formUrl) {
      const flatPermission = flatMap(this.$store.getters.permissionList, e => [e, ...(e.children || [])])
      const routeConfig = flatPermission.filter(e => e.path === this.$route.path)[0] || {}
      this.formUrl = (routeConfig.component || '').replace(/(.*)\/(.*?)List/, '$1/modules/$2Form')
      if (!this.formUrl) {
        throw new Error('无法装载formUrl,请检查data.formUrl')
      }
    }
    if (!this.getFlowCode()) {
      throw new Error('无法装载flowCode,请检查data.flowCode或实现getFlowCode()方法')
    }
    if (!this.getProcessDefinitionId()) {
      throw new Error('无法装载processDefinitionId,请检查data.processDefinitionId或实现getProcessDefinitionId()方法')
    }
    console.log('BindBpmMixin', this.path, this.formUrl, this.getFlowCode());
  },
  methods: {
    loadData(arg) {
      if (!this.url.list) {
     ...
     */
    getFlowCode(record = {}) {
      console.log('获取flowCode,如果是多流程列表,可能需要复写此方法', this.flowCode, record)
    ...
    getProcessDefinitionId() {
      console.log('获取processDefinitionId,如果是多流程列表,可能需要复写此方法', this.processDefinitionId)
    ...
    async combineBpmDataList() {
      ...
      const recordsWithTask = intersectionWith(this.dataSource, this.myTaskList, (record, task) => {
    ...
    loadBpmData(val) {
      ...
      const promises = this.dataSource.map(item => item.bpmStatus === '2' ? getAction(this.url.getBizProcessNodeInfo, {
        flowCode: this.getFlowCode(),
        dataId: item.id
      }) : Promise.resolve({ success: false }));
      Promise.all(promises).then((arr) => {
        arr.forEach((result, idx) => {
          result.success && this.$set(this.dataSource[idx], 'bpmData', result.result)
        })
        console.log('loadBpmData', arr, this.dataSource);
      })
    },
    /**
     * 展示流程-我的任务处理面板
    ...
    editProcessTrack(params) {
      this.$refs.bindBpm.$refs.bpmProcessTrackModal.handleTrack(params);
      this.$refs.bindBpm.$refs.bpmProcessTrackModal.title = '编辑权限申请-审批跟踪记录';
    },
    /**
     * ==============第三方模块功能============
     * 获取受控编辑按钮的参数
     * @param record
     */
    getEditParams(record) {
      return {
        record,
        id: record.id,
        bpmTitle: `编辑权限申请`,
        bpmFlowCode: this.getFlowCode(record),
        bpmFormUrl: this.formUrl
      };
    }
  }
}
