<template>
  <a-card :bordered="false" style="height: 100%">
    ...
    <div>
      ...
      <!--嵌入流程审批组件-->
      <task-module-for-online v-if="showDealBlock" :save-form="preSaveForm" :formData="onlineFormData" @complete="$emit('complete')" />
    </div>
  </a-card>
</template>
...
export default {
    ...
    props: {
      ...
      // 展示查询模块
      onlineShowQueryBlock: {
        type: Boolean,
        default: true
      },
      // 初始化筛选参数
      onlineInitQueryParam: {
        type: Object,
        default() {
          return {}
        }
    ...
    methods: {
      ...
      initOnlineBpmData() {
        // 先加载props配置
        this.code = this.onlineFormConfig.code
