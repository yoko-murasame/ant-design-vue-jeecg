<template>
  <a-card v-bind="cartAttrs">
    ...
    <!--查询插槽-->
    <div class="table-page-search-wrapper" v-if="$slots.cardQuery || $scopedSlots.cardQuery">
    ...
    <!--标题插槽 title部分-->
    <span slot="title" v-if="$slots.cardTitle || $scopedSlots.cardTitle">
      <slot name="cardTitle">
        <span>这是标题</span>
      </slot>
    </span>
    <div slot="extra" class="d-flex df-jc-center df-ai-center" v-if="$slots.cardExtra || $scopedSlots.cardExtra">
    ...
    <!-- 操作按钮区域 -->
    <div class="table-operator" v-if="$slots.cardButton || $scopedSlots.cardButton">
    ...
    <div>
      ...
      <!--选择区域插槽-->
      <div class="ant-alert ant-alert-info" style="margin-bottom: 16px;" v-if="$slots.cardRowSelector || $scopedSlots.cardRowSelector">
    ...
    <!--Card底部按钮部分插槽-->
    <template slot="actions" v-if="$slots.cardBottomButton || $scopedSlots.cardBottomButton">
...
export default {
    ...
    watch: {
      'onlineFormConfig.code': {
        handler(v) {
          if (v) {
            console.log('Online自动列表加载::OnlCgformAutoList::initAutoList::监听onlineFormConfig.code')
            this.resetConfig()
