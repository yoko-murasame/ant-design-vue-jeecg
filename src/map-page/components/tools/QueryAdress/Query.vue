...
<script>
import { gcj02towgs84 } from '@/map-page/components/tools/QueryAdress/point'
  ...
  methods: {
    ...
    AddPoint() {
      if (this.Query_List.length > 0) {
        let AddressPoint = []
        console.log('高德->国家2000，坐标转换前:', JSON.parse(JSON.stringify(this.Query_List)))
        for (let k in this.Query_List) {
          ...
          // ])
          // 测试转换
          let lngLat = gcj02towgs84(Number(this.Query_List[k]['location'].lng),
            Number(this.Query_List[k]['location'].lat))
        ...
        console.log('高德->国家2000，坐标转换后:', JSON.parse(JSON.stringify(AddressPoint)))
        let features = {
